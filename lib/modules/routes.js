...
var sendWorkout = {
    ...
    validate :{
        payload: {
            personId: Joi.string().alphanum().min(3).max(20),
            date: Joi.date(),
            exercise: Joi.string().alphanum().min(3).required(),
            musclegroup: Joi.string().alphanum().min(3).required(),
            reps: Joi.array().min(1).required(),
            kilos: Joi.array().min(1).required()
        }
    }
};
